...
Set **one** key in Vercel → Settings → Environment Variables. OpenAI wins if both are set, matching `api/chat.js` and `api/decide.js`.

| Var | Value | Notes |
|---|---|---|
| `OPENAI_API_KEY` | `sk-proj-…` | **The house default.** `api/chat.js` and `api/decide.js` also run on it, so this endpoint prefers it — one key, one provider across the app. |
| `OPENAI_MODEL` | optional | Defaults to `gpt-4o`. Shared with `api/chat.js` and `api/decide.js`. |
| `ANTHROPIC_API_KEY` | `sk-ant-api03-…` | Fallback only — used when `OPENAI_API_KEY` is unset. Model `claude-opus-4-8`. |
| `DRAFT_MODEL` | optional | Overrides the model for this endpoint only. |
