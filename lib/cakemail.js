...
//
// `stopWhen(rows, all)` ends paging early — return true and the pages after it are not
// requested. Sorting is -created_on, so anything a caller already holds is at the BACK: once a
// whole page contains nothing new, every later page is older and also known.
//
// This is what makes a routine sync cheap. Without it, discovering that nothing has changed
// costs a full walk of the account — 7 round trips for 302 campaigns, ~5.7s — every single
// time, which is what made the Market History tab feel like it was loading for six seconds
// when it had in fact already rendered (Vhea, 2026-09-08).
export async function listCampaigns({ accountId, key, status = 'delivered', perPage = 50, maxPages = 40, stopWhen = null } = {}) {
  key = key || cakemailKey(accountId);
  if (!key) throw new Error(`cakemail: no API key for account ${accountId} — set ${cakemailKeyEnvName(accountId)} on the server`);
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    // Asked AFTER a full page, never after a short one: a short page is already the end, and
    // reporting that as "stopped early" would make a complete listing look partial.
    if (stopWhen && stopWhen(rows, out)) { complete = false; break; }
  }
  // Non-enumerable so it cannot show up in JSON.stringify or a spread of the array.
