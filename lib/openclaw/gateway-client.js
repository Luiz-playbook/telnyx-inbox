...
export async function runAgentTurn({ sessionKey, text, agentId, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  ...
  // CONFIRMED via CLI: the gateway requires the sessionKey to encode the agentId as
  // `agent:<agentId>:<name>` (rejects a mismatch). Build it from the caller's per-user token.
  const userPart = String(sessionKey).replace(/^agent:[^:]*:/, '').replace(/[^A-Za-z0-9._-]/g, '-') || 'default';
  sessionKey = `agent:${agentId}:${userPart}`;
    ...
    ws.onmessage = async (ev) => {
      ...
      if (m.event === 'connect.challenge' && !connected) {
        ...
        try {
          ...
          chatId = randomUUID();
          // CONFIRMED via CLI: required params sessionKey (agent:<id>:<name>), agentId, message,
          // idempotencyKey. The ack res is { runId, status:"started" } where runId === idempotencyKey;
          // the reply then streams as events correlated by runId.
          const idem = randomUUID();
          activeRunId = idem;
          pending.set(chatId, { res: (p) => { if (p && p.runId) activeRunId = p.runId; }, rej: (e) => finish(e) });
          send({ type: 'req', id: chatId, method: 'chat.send', params: {
            sessionKey, agentId, message: text, idempotencyKey: idem,
          } });
        } catch (e) {
          ...
          if (/approv|pending|device.*(not.*approv|unauthori)/i.test(msg)) {
            // approve takes the PENDING REQUEST id (from `openclaw devices list`), not the device id.
            finish(new Error('device_pending_approval: on the gateway run `openclaw devices approve --latest` (or `openclaw devices list` to find the request id), then retry'));
          } else finish(e);
        }
        return;
      }
