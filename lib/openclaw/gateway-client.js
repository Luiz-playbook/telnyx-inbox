  ...
  return await new Promise((resolve, reject) => {
    ...
    function processReply(event, p) {
      ...
      if (event === 'chat') {
        ...
        if (p.state === 'final') { finish(null, { reply: (full || buf).trim(), runId: ackRunId || runId }); return; }
      ...
      if (event === 'agent' && p.stream === 'lifecycle' && p.data?.phase === 'end') {
        if (buf.trim()) finish(null, { reply: buf.trim(), runId: ackRunId || runId });
      }
    }
