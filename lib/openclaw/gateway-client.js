...
export async function runAgentTurn({ sessionKey, text, agentId, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    function processReply(event, p) {
      ...
      // Only the `chat` stream is authoritative. The parallel `agent` lifecycle `end` fires BEFORE
      // the last `chat` delta + the `chat` final, so finishing on it truncates the reply — ignore
      // `agent.*` entirely. Each `chat` event carries the CUMULATIVE message text (not just the
      // increment), so prefer that snapshot; `deltaText` is only a fallback when no message is set.
      if (event !== 'chat') return;
      const full = messageText(p.message);
      if (full) buf = full;
      else if (typeof p.deltaText === 'string') buf += p.deltaText;
      if (p.state === 'final') { finish(null, { reply: buf.trim(), runId: ackRunId || runId }); return; }
      if (p.state === 'error') { finish(buf.trim() ? null : new Error('agent error'), buf.trim() ? { reply: buf.trim(), runId: ackRunId || runId } : undefined); return; }
