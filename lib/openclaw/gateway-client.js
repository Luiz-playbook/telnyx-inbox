...
export async function runAgentTurn({ sessionKey, text, agentId, timeoutMs = DEFAULT_TIMEOUT_MS, onDelta } = {}) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    function processReply(event, p) {
      // Fail CLOSED on attribution. The old rule ("drop it only if it has a runId and the runId is
      // wrong") waved through every unstamped event — and the gateway's history replay includes
      // `chat` events that reach us without one. A replayed empty `final` ended the turn as
      // `(no reply)`; a replayed final WITH text would have returned the previous turn's answer as
      // this one's, which looks right and isn't. So once our runId is known, an event counts as ours
      // only if it says so. (If the ack somehow carried no runId we have nothing to compare against;
      // that path stays permissive rather than dropping every event — see the ack handler.)
      if (ackRunId && p.runId !== ackRunId) return; // history / other runs
      ...
      // `agent.assistant` stream into a SEPARATE buffer used only as a fallback (it must carry our
      // runId too; an unstamped assistant frame is dropped by the filter above rather than risking
      // stale text in the fallback — losing a fallback beats answering with the wrong turn) — adding it to
