...
// Read keyed on SERVICE_NAME, matching storeTokens. This used to filter on client_id, so
// re-registering the Salesmsg app (a new client_id) made the stored token invisible and
// silently fell back to the env var — the same class of bug as the write side.
async function readStoredRefresh() {
  const h = serviceHeaders(), url = process.env.SUPABASE_URL;
  if (!h || !url) return null;
  const r = await fetch(
    `${url}/rest/v1/salesmsg_secrets?service_name=eq.salesmsg&select=refresh_token&order=last_updated.desc&limit=1`,
    { headers: h });
  if (!r.ok) return null;
...
// Upsert keyed on SERVICE_NAME, not client_id.
//
// salesmsg_secrets carries a UNIQUE constraint on service_name, so there is exactly one
// row per integration — the table's own answer to "which credentials is this app using".
// This used to look the row up by client_id and INSERT when it found none, which meant a
// new app registration (client 2037, where the old row held 1188) hit
// `23505 duplicate key ... salesmsg_secrets_service_name_key` and returned false. The
// caller treats false as "nowhere durable to write", so every rotated refresh token was
// silently discarded and the grant died a few days later, repeatedly. The failure was
// invisible because nothing surfaced the 409.
//
// Rotating the app registration therefore UPDATES the row, carrying the new client_id
// with it. Rows for other service_names are untouched.
export async function storeTokens({ refreshToken, accessToken, expiresAt }) {
  ...
  // on_conflict + merge-duplicates makes this one atomic upsert against the unique key,
  // so a concurrent refresh cannot land between a lookup and a write.
  const r = await fetch(
    `${url}/rest/v1/salesmsg_secrets?on_conflict=service_name`,
    { method: 'POST',
...
export async function sendSmsBulk({ teamId, to = [], message, concurrency = 8 }) {
  ...
  // `sent` stays a COUNT because existing callers read it as one. `sentNumbers` is the
  // same information as a list, added for AI-976: HubSpot has to be told which numbers
  // actually received a message, and a count cannot say that.
  return { sent: sent.length, sentNumbers: sent, failed, total: list.length };
}
