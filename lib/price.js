...
export function buildPricePrompt(games) {
  ...
  // Price rules stay together and come FIRST. When the URL instructions were interleaved with
  // them, accuracy collapsed: the run average went from ~$28 to $42 and one game came back at
  // $1,189 as a "get-in" price — a premium seat, not the cheapest. The URL is secondary and is
  // asked for last, explicitly as optional, so it cannot compete with the price rules.
  return [
    ...
    '- The get-in price is the CHEAPEST ticket available for that game, in the worst seat in',
    '  the stadium. It is NOT a good seat, a premium/club/suite seat, or an average price.',
    ...
    '- A regular-season get-in is usually $5-$80. If your number is in the hundreds you have',
    '  found a premium seat, not the get-in — look again for the cheapest listing.',
    '- Do NOT guess: if you cannot find a price, return price_usd = null.',
    'Secondary, optional — never let this affect the price:',
    '- "url": the link to that game\'s listing page, if you actually have one. Return null',
    '  rather than constructing or guessing a URL; a wrong link is worse than none.',
