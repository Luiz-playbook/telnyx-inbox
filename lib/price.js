...
export async function callPrices(allGames) {
  ...
  if (!route.ok) return { ok: false, ms: 0, priced: [], inTok: 0, outTok: 0, costUsd: null, route, retried: 0, skipped: 0 };
  ...
  if (!r.ok) return { ok: false, ms: Date.now() - t0, priced: [], inTok: 0, outTok: 0, costUsd: null, route, retried: 0, skipped };
  ...
  return { ok: true, ms: Date.now() - t0, priced, inTok, outTok, costUsd, route, retried, skipped };
}
