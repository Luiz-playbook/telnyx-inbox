...
// Per-request cost on the OpenRouter route, for the pre-run estimate only. Perplexity bills
// per web search rather than a flat grounding fee, so there is no published per-request number
// to quote — this is measured from real runs and should be re-measured if the model changes:
//   2026-08-27 00:01  13 batches  $0.2716  -> $0.0209
//   2026-08-27 12:01  11 batches  $0.2485  -> $0.0226
// Only the ESTIMATE uses this. The run log records the exact charge OpenRouter reports per
// response, so what gets stored is never this approximation.
export const OR_COST_PER_REQ = 0.022;
...
// "402 Insufficient credits" from a failed provider response. Status first, then the first
// sentence of whatever message the provider sent — short enough for a tooltip, specific enough to
// act on. Shown to operators in the price editor (AI-969), so it must never be a bare "failed".
const HTTP_TEXT = { 400: 'Bad request', 401: 'Unauthorized', 402: 'Payment required', 403: 'Forbidden',
  404: 'Not found', 408: 'Timed out', 429: 'Too many requests', 500: 'Server error', 502: 'Bad gateway',
  503: 'Unavailable', 504: 'Gateway timeout' };
export function httpErrorText(status, json) {
  const raw = json?.error?.message || json?.message || (typeof json?.error === 'string' ? json.error : '') || '';
  const first = String(raw).split(/(?<=\.)\s/)[0].replace(/\.$/, '').trim();
  return `${status} ${first || HTTP_TEXT[status] || 'Error'}`.slice(0, 120);
}
...
async function fetchPrices(route, games) {
  ...
  if (route.via === 'google') {
    ...
    if (!res.ok) return { ok: false, ms: Date.now() - t0, text: '', inTok: 0, outTok: 0, costUsd: null, error: httpErrorText(res.status, json) };
...
export async function callPrices(allGames) {
  const route = priceRoute();
  if (!route.ok) return { ok: false, ms: 0, priced: [], inTok: 0, outTok: 0, costUsd: null, route, retried: 0, skipped: 0, error: 'No AI search key set (OPENROUTER_GEMINI / GEMINI_API_KEY)' };

  // REF INTEGRITY. Every row is matched back by external_id, and buildPricePrompt writes it into
  // the prompt as "ref=<id>". A game without one renders "ref=undefined", and the damage is not
  // limited to that row: observed live, the model then abandoned the ref scheme for the WHOLE
  // batch — echoing the literal string "undefined", or substituting its own description — so
  // every price came back unmatchable and was dropped. Full cost, zero coverage, no error.
  //
  // Cheap to prevent, so prevented here rather than trusted to callers: rows with no id are
  // dropped before the prompt is built, and counted so the caller can see it happened.
  const games = (allGames || []).filter(g => g && g.external_id != null && String(g.external_id).trim() !== '');
  const skipped = (allGames || []).length - games.length;
  if (!games.length) return { ok: false, ms: 0, priced: [], inTok: 0, outTok: 0, costUsd: null, route, retried: 0, skipped };

  const t0 = Date.now();
  // A network failure used to throw straight out of here and take the whole refresh with it.
  // It is a failed call like any other: reported, not raised.
  const r = await fetchPrices(route, games).catch(e => ({ ok: false, error: `Network error: ${String(e.message || e).slice(0, 80)}` }));
  if (!r.ok) return { ok: false, ms: Date.now() - t0, priced: [], inTok: 0, outTok: 0, costUsd: null, route, retried: 0, skipped, error: r.error };
  ...
  if (route.via === 'openrouter' && fallbackModel && fallbackModel.toLowerCase() !== 'off'
    ...
    const r2 = await fetchPrices({ ...route, model: fallbackModel }, missing).catch(e => ({ ok: false, error: `Network error: ${String(e.message || e).slice(0, 80)}` }));
    if (r2.ok) {
      ...
      fallbackError = r2.error;
    }
  }

  // fallbackError: why the retry could not price the games the first pass missed. A game still
  // unpriced after an OK call and no fallback error simply had no usable listing found.
  return { ok: true, ms: Date.now() - t0, priced, inTok, outTok, costUsd, route, retried, skipped, fallbackError };
}
